...
def generate_engagement():
    ...
    data = pd.DataFrame(columns = ['alias','user_insta','user_tt','followers_tt','total_likes_5days',
                                   'like_mean_5days','total_rts_3days','rt_mean_3days','score','date'], index=players_names)
    ...
    data['rt_mean_3days'] = data.apply(lambda x: count_rt_likes(x.user_tt)['mean_rts'], axis = 1)
    data['total_likes_5days'] = data.apply(lambda x: count_rt_likes(x.user_tt)['total_likes'], axis = 1)
    data['total_rts_3days'] = data.apply(lambda x: count_rt_likes(x.user_tt)['total_rts'], axis = 1)


    # data['sentiment'] = data.apply(lambda x: get_sentiment(['{} #BBB22'.format(x.alias),'{} #bbb22'.format(x.alias)], 500), axis=1)

    #new columns
    data['rt_follower'] = data['total_rts_3days']/data['followers_tt']
